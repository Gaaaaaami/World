...
public:
    ...
    void GenerateAllChunks( UProceduralMeshComponent* InProduralMeshComponet = nullptr, FPlanetChunk* InPlanetChunk = nullptr, TObjectPtr<URHIDensityTools> InRHIDT = nullptr);
    
    /** Generate a single chunk at the specified grid position */
    bool GenerateChunk(int32 X, int32 Y, int32 Z, FVector ChunkCenter, UProceduralMeshComponent * InProduralMeshComponet = nullptr, FPlanetChunk* InPlanetChunk = nullptr, TObjectPtr<URHIDensityTools> InRHIDT = nullptr);
    bool RHIGenerateChunk(int32 X, int32 Y, int32 Z, FVector ChunkCenter, UProceduralMeshComponent* InProduralMeshComponet = nullptr, FPlanetChunk* InPlanetChunk = nullptr, TObjectPtr<URHIDensityTools> InRHIDT = nullptr);
    bool GenerateMesh(const TArray<float> &InDensityField, FVector3f Location, FVector InChunkCenter);
public:
    UPROPERTY()
