	...
	for (int32 z = Padding; z < (int32)total.z - Padding; ++z)
	{
		for (int32 y = Padding; y < (int32)total.y - Padding; ++y)
		{
			for (int32 x = Padding; x < (int32)total.x - Padding; ++x)
			{
				...
				{
					FString n;
					n = n.Printf(TEXT("%d"), int(Weight));
					DrawDebugString(this->GetWorld(), worldOffset + FVector(x * this->VoxelSize, y * this->VoxelSize, z * this->VoxelSize), n, 0, FColor::White, -1.f, true, 1.f);
					DrawDebugSphere(this->GetWorld(), worldOffset + FVector(x * this->VoxelSize, y * this->VoxelSize, z * this->VoxelSize), 2.f, 0, FColor::Red, true);
				}
				else if (x == 15 && this->LandscapeDimension == FVector(2))
				{
					FString n;
					n = n.Printf(TEXT("[%d, %d, %d] %d"),x,y,z, int(grid(x, y, z).weight));
